...
namespace JMW.Functional
{
    ...
    public struct Optional<T> where T : class
    {
        ...
        public void Do(Action<T> fn, Action if_null)
        {
            if (Exists) fn(Value);
            else if_null();
        }
        ...
        public Optional<R> Do<R>(Func<T, R> fn) where R : class
        {
            if (Exists)
        ...
        /// <typeparam name="R">The 1st type parameter.</typeparam>
        public R Do<R>(Func<T, R> fn, Func<R> if_null) where R : class
        {
            if (Exists)
                return fn(Value);
